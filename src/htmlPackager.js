...
class HtmlPackager extends HTMLPackager {
  ...
  async addAsset(asset) {
    ...
    // keep only css and js files to include in <head>
    // remove files generated by svg import
    const headBundles = siblingBundles.filter(
      b => ['css', 'js'].includes(b.type) && b.parentBundle.name.slice(-4) !== '.svg',
    );
   ...
   */
  getSvgAssets() {
    // only svg loaded by the plugin file have 'generated.svg' property
